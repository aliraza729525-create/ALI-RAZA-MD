        ...
        if (isBanned(senderId) && !userMessage.startsWith('.unban')) {
            ...
            if (Math.random() < 0.1) {
                await sock.sendMessage(chatId, {
                    text: '🛑 ALI RAZA BOT: You’ve been officially blocked from commands. Talk to an admin.',
                    ...channelInfo
                });
            }
            return;
        ...
        if (isGroup && isAdminCommand) {
            ...
            if (
                ...
                if (!isSenderAdmin && !message.key.fromMe) {
                    await sock.sendMessage(chatId, {
                        text: 'ALI RAZA BOT: Nice try, peasant. This button belongs to the kings and queens (admins).',
                        ...channelInfo
                    });
                    return;
                }
        ...
        switch (true) {
            case userMessage === '.simage': {
                ...
                if (quotedMessage?.stickerMessage) {
                    ...
                    await sock.sendMessage(chatId, { text: '🎯 Tip from ALI RAZA BOT: Just reply to any sticker with .simage and watch the magic happen ✨.', ...channelInfo });
                }
                break;
            }
            ...
            case userMessage.startsWith('.mute'):
                ...
                if (isNaN(muteDuration)) {
                    await sock.sendMessage(chatId, { text: '🕒 ALI RAZA BOT Tip: Minutes must be valid! Try: .mute 10\neg to mute 10 minutes\n.mute 10', ...channelInfo });
                } else {
...
case (userMessage && userMessage.trim().toLowerCase() === '.whoisgay'):
  ...
  break;
            ...
            case userMessage.startsWith('.mode'):
                ...
                try {
                    ...
                    await sock.sendMessage(chatId, { text: `🔄 Mode updated — ALI RAZA BOT is now in *${action}* mode.`, ...channelInfo });
                } catch (error) {
  ...
  break;
            ...
            case userMessage.startsWith('.antilink'):
                if (!isGroup) {
                    await sock.sendMessage(chatId, {
                        text: '🏠 ALI RAZA BOT: This command works only inside groups.',
                        ...channelInfo
                    });
                    return;
                }
                if (!isBotAdmin) {
                    await sock.sendMessage(chatId, {
                        text: '⚠️ ALI RAZA BOT: Make me an admin first so I can do my magic. 🪄',
                        ...channelInfo
                    });
                    return;
                }
...
case userMessage === '.fight' || userMessage === '.battle' || userMessage === '.vs':
  ...
  break;
            ...
            case userMessage === '.ALI RAZAsupport':
         ...
         // .explode
            ...
            case userMessage.startsWith('.move'):
                ...
                if (isNaN(position)) {
                    await sock.sendMessage(chatId, { text: '🕹 ALI RAZA BOT says: That’s not a valid position! Pick a real one.', ...channelInfo });
                } else {
  ...
  break;
            case userMessage.startsWith('.answer'):
                ...
                if (answer) {
                    ...
                    sock.sendMessage(chatId, { text: '💬 ALI RAZA BOT: Provide your answer using .answer <answer>', ...channelInfo });
                }
                break;
    ...
    break;
            ...
            case userMessage.startsWith('.welcome'):
                if (isGroup) {
                    ...
                    await sock.sendMessage(chatId, { text: '🏠 ALI RAZA BOT: This command can only be used inside groups...', ...channelInfo });
                }
                break;
            ...
            case userMessage.startsWith('.antibadword'):
                ...
                if (!isBotAdmin) {
                    await sock.sendMessage(chatId, { text: '*👑 Give ALI RAZA BOT admin powers before trying this!*', ...channelInfo });
                    return;
                }
            ...
            case userMessage.startsWith('.chatbot'):
                if (!isGroup) {
                    await sock.sendMessage(chatId, { text: '👥 ALI RAZA BOT says: You gotta be in a group to run this..', ...channelInfo });
                    return;
                }
                ...
                if (!chatbotAdminStatus.isSenderAdmin && !message.key.fromMe) {
                    await sock.sendMessage(chatId, { text: '*🚫 ALI RAZA BOT: Only group admins or the bot owner can use this command. 👑*', ...channelInfo });
                    return;
                }
...
// Update commands for update bot auto 
     ...
     //           break;
            ...
            case userMessage === '.groupinfo' || userMessage === '.infogp' || userMessage === '.infogrupo':
                if (!isGroup) {
                    await sock.sendMessage(chatId, { text: '👥 ALI RAZA BOT says: You must be in a group to use this.', ...channelInfo });
                    return;
                }
...
case userMessage.startsWith('.sudo'):
                ...
                async function groupJidCommand(sock, chatId, message) {
                    ...
                    if (!groupJid.endsWith('@g.us')) {
                        return await sock.sendMessage(chatId, {
                            text: "👥 ALI RAZA BOT says: You must be in a group to use this."
                        });
                    }
        ...
        if (chatId) {
            await sock.sendMessage(chatId, {
                text: '💥 Error! ALI RAZA BOT couldn’t complete your request.',
                ...channelInfo
            });
        }
    }
...
async function handleGroupParticipantUpdate(sock, update) {
    try {
        ...
        if (action === 'add') {
            ...
            const welcomeMessage = welcomeData?.message || '👑 ALI RAZA BOT: Hail {user}! The group just got cooler with you here!';
